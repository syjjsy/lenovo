...
from erfnet import PyramidPooling
...
class resnet_wide(nn.Module):
    def __init__(self,num_in_layers):
        ...
        self.resnet50_1=Resnet50_md(1)
        self.resnet50_2=Resnet50_md(1)

        self.disp1_layer = get_disp(32)
        self.diap_out = nn.Conv2d(2,1,1)

    def forward(self,x):
        out1=self.resnet50_1(x)
        out2=self.resnet50_2(x)
        # print(out1.shape)
        # out=out1.mul(out2)
        out=torch.cat((out1, out2), 1)
        # print(out.shape)

        disp1 = self.disp1_layer(out)
        # print("disp1",self.disp1.shape)
        out = self.diap_out(disp1)
       
        # print(out.shape)
        return out

class Resnet_psp_md(nn.Module):#resnet 101
    def __init__(self, num_in_layers):
        super(Resnet_psp_md, self).__init__()
        # encoder
        self.conv1 = conv(num_in_layers, 64, 7, 1)  # H/2  -   64D
        #self.pool1 = maxpool(3)  # H/4  -   64D
        self.conv2 = resblock(64, 64, 3, 1)  # H/8  -  256D
        self.conv3 = resblock(256, 128, 4, 1)  # H/16 -  512D
        self.conv4 = resblock(512, 256, 6, 1)  # H/32 - 1024D##################################6
        self.conv5 = resblock(1024, 512, 3, 1)  # H/64 - 2048D
        ...
        self.upconv1 = upconv(32, 16, 3, 2)
        self.iconv1 = conv(16+2, 16, 3, 1)
        self.disp1_layer = get_disp(4)
        self.diap_out = nn.Conv2d(2,1,1)

        self.convps = nn.Conv2d(16,16*8*8,3,1,padding=1)
        self.convpss = nn.Conv2d(16,16*8*8,3,1,padding=1)
        self.ps=nn.PixelShuffle(8)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.xavier_uniform_(m.weight)
        self.psp_layer1 = PyramidPooling('psp', 128, 1536,
                                        norm_layer=nn.BatchNorm2d)
        self.psp_layer2 = PyramidPooling('psp', 66, 640,
                                        norm_layer=nn.BatchNorm2d)
        self.psp_layer3 = PyramidPooling('psp', 34, 322,
                                        norm_layer=nn.BatchNorm2d)
        self.psp_layer4 = PyramidPooling('psp', 256, 98,
                                        norm_layer=nn.BatchNorm2d)
    ...
    def forward(self, x):
        ...
        upconv4=self.psp_layer1(concat5)
        concat4 = torch.cat((upconv4, x3), 1)

        upconv3=self.psp_layer2(concat4)
        concat3 = torch.cat((upconv3, x2), 1)

        upconv2=self.psp_layer3(concat3)
        concat2 = torch.cat((upconv2, x1), 1)
    
        iconv1=self.psp_layer4(concat2)
        iconv1 = self.ps(iconv1)
        # print(iconv1.shape)
