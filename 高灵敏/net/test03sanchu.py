...
def return_arguments():
    ...
    parser.add_argument('--loadmodel', 
                        default='./model2pose-butongweizhi/0114_data_cpt_cpt.pth',
                    help='load model')                    
    ...
    def test(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for data in self.loader:       #(test-for-train)
                ...
                leftimag = os.path.join('./xiaoguosanchu/', str('85_2000_8-0225_%06d' % i) + '-.bmp')
                bimag=bg_image.cpu().numpy().squeeze()
                imageio.imwrite(leftimag, bimag)
                disp_showa = disp_show.data.cpu().detach().numpy()
                dst = os.path.join('./xiaoguosanchu/', str('85_2000_8-0225_%06d' % i) + '.bmp')
