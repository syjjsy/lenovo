...
def return_arguments():
    ...
    parser.add_argument('--data_dir',
                        ...
                        default='./端到端-butongweizhi/test/'
                        )
    parser.add_argument('--loadmodel', default='./model256/0218_dataTO_512_cpt_cpt.pth',
                    help='load model')                    
    ...
    def test(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for data in self.loader:       #(test-for-train)
                ...
                #leftimag = os.path.join('./xiaoguo80_down/',str('85_1024down784_256_%06d' % i) + '-.bmp')
                #####bimag=bg_image.cpu().numpy().squeeze()
                #imageio.imwrite(leftimag, bimag)
                disp_showa = disp_show.data.cpu().detach().numpy()
                dst = os.path.join('./0908/', str('85_1024down784_256_%06d' % i) + '.bmp')
