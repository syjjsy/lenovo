...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        cost0 = self.dres1(cost0) + cost0#######64 to 32 channels   Resnet

        out1, pre1, post1 = self.dres2(cost0, None, None) 
        out1 = out1+cost0     #######1x32x48x64x128
        #print(out1.shape)
        out2, pre2, post2 = self.dres3(out1, pre1, post1) 
        out2 = out2+cost0           #######1x32x48x64x128
        #print(out2.shape)
        out3, pre3, post3 = self.dres4(out2, pre1, post2) 
        out3 = out3+cost0                  #######1x32x48x64x128
        
        cost1 = self.classif1(out1)    #NO RELU    1x1x48x64x128
        
        cost2 = self.classif2(out2) + cost1   #1x1x48x64x128
      
        cost3 = self.classif3(out3) + cost2     #1x1x48x64x128
       

        if self.training:
            cost1 = F.upsample(cost1, [self.maxdisp,left.size()[2],left.size()[3]], mode='trilinear')     #[1,1,192,256,512]
            cost1 = torch.squeeze(cost1,1)                                                                 #[1,192,256,512]
            pred1 = F.softmax(cost1,dim=1)                                                                 #[1,192,256,512]
            pred1 = disparityregression(self.maxdisp)(pred1)                                               #[1,1,256,512]
